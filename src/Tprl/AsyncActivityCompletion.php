...
use Temporal\Utils\Logger;


// @@@SNIPSTART samples-php-async-activity-completion-activity-class
class AsyncActivityCompletion implements AsyncActivityCompletionInterface
{
     ...
     */
    public function composeCompletion(string $greeting, string $name): string
    {
        // In real life this request can be executed anywhere. By a separate service for example.
        $this->logger->info(sprintf('AsyncActivityCompletion token: %s', base64_encode(Activity::getInfo()->taskToken)));
